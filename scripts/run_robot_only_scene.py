#!/usr/bin/env python3
"""Launch the Zhishu robot-only scene and keep it idle."""
...
def main() -> None:
    ...
    try:
        ...
        if args.start_paused:
            ...
            timeline.pause()
            print("[INFO] robot-only scene loaded and timeline paused; press Play in Isaac Sim.")

        zero_action = torch.zeros((env.num_envs, env.action_dim), device=env.device)
        step_idx = 0
        print("[INFO] robot-only scene ready; stepping zero actions until you close the app or Ctrl-C.")
