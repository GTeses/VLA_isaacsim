#!/usr/bin/env python3
"""Launch the Zhishu robot-with-plate scene and keep it idle."""
...
def main() -> None:
    ...
    try:
        ...
        from zhishu_dualarm_lab.tasks.robot_with_plate.env import ZhishuDualArmRobotWithPlateEnv
        from zhishu_dualarm_lab.tasks.robot_with_plate.env_cfg import ZhishuDualArmRobotWithPlateEnvCfg

        cfg = ZhishuDualArmRobotWithPlateEnvCfg()
        cfg.scene.num_envs = args.num_envs
        env = ZhishuDualArmRobotWithPlateEnv(cfg=cfg, render_mode="human" if not args.headless else None)
        env.reset()
        timeline = None
        if args.start_paused:
            ...
            timeline.pause()
            print("[INFO] robot-with-plate scene loaded and timeline paused; press Play in Isaac Sim.")

        zero_action = torch.zeros((env.num_envs, env.action_dim), device=env.device)
        step_idx = 0
        print("[INFO] robot-with-plate scene ready; stepping zero actions until you close the app or Ctrl-C.")
