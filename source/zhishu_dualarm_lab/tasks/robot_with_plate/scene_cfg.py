...
from zhishu_dualarm_lab.tasks.dualarm_tabletop.cameras import (
    build_external_camera_cfg,
    build_left_wrist_camera_cfg,
    build_right_wrist_camera_cfg,
    build_waist_camera_cfg,
)
from zhishu_dualarm_lab.tasks.dualarm_tabletop.objects import build_ground_cfg, build_light_cfg
from zhishu_dualarm_lab.utils.tcp_frames import make_tcp_frame

from .objects import build_plate_cfg
...
@configclass
class ZhishuDualArmRobotWithPlateSceneCfg(InteractiveSceneCfg):
    """Scene with the robot, cameras, TCP frames, and a fixed plate prop."""

    ground = build_ground_cfg()
    dome_light = build_light_cfg()
    robot = build_zhishu_robot_cfg()
    plate = build_plate_cfg()
